...
def freeze_bn(module):
    """
    Puts BatchNorm layers within `module` into eval mode, independent of
    requires_grad, so their running stats stop updating. Needed because
    requires_grad=False alone does NOT stop BatchNorm running-stat drift —
    model.train() (called at the top of every train_epoch) re-enables BN
    training mode on the whole model regardless of which params are frozen.
    Opt-in only: called from train_epoch iff freeze_bn_modules is passed.
    """
...
def get_model(data, device):
    ...
    ### 2G BoT LBS
    # FIX: this was unconditional in the upstream source (no matching
    # `if data['model_arch']=='...':` guard) — it ran on every call to
    # get_model() regardless of model_arch, silently overwriting whatever
    # model the correct branch above had just built. Confirmed as a real,
    # blocking bug (not a scraping artifact) once MBR_4B tried to run and
    # got clobbered by this line instead. Gated here behind 'MBR_BOT_2G',
    # inferred from the naming convention used by every neighboring branch
    # (MBR_BOT_2B exists for 2-branch BoT+LBS; MBR_R50_2G exists for
    # 2-group R50+LBS) — this is a reconstruction, not a confirmed original
    # condition, since the true guard (if one ever existed) wasn't
    # recoverable. If you know the real intended trigger, swap it in here.
    if data['model_arch'] == 'MBR_BOT_2G':
...
def train_epoch(model, device, dataloader, loss_fn, triplet_loss, optimizer, data,
                 alpha_ce, beta_tri, logger, epoch, scheduler=None, scaler=False,
                 freeze_bn_modules=None):
    # Set train mode for both the encoder and the decoder
    model.train()

    # Opt-in: re-freeze BatchNorm layers in the given submodule(s) AFTER
    # model.train() above, since model.train() unconditionally re-enables BN
    # training mode across the whole model. Only fires when a caller passes
    # freeze_bn_modules explicitly (e.g. during an epoch_freeze_L1toL3
    # warmup period) — default None means this is a complete no-op and
    # existing call sites that don't pass it are unaffected.
    if freeze_bn_modules is not None:
        freeze_bn(freeze_bn_modules)
